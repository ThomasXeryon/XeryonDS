...
COMMAND_TIMEOUT = 3.0  # 3 seconds timeout for commands
ULTRASAFE_MODE = True  # Enable additional safety checks
...
logging.basicConfig(
    ...
    handlers=[
        ...
        logging.FileHandler('/tmp/xeryon_client.log') if RUNNING_ON_RPI else logging.NullHandler()
    ]
)
...
def initialize_xeryon_controller():
    ...
    try:
        ...
        if not flush_serial_port():
            logger.error("Failed to flush serial port - aborting controller init")
            return False
...
def initialize_camera():
    ...
    try:
        ...
        # Capture a few test frames to make sure camera is working
        for i in range(3):
...
async def process_command(data):
    ...
    logger.debug(f"Command received: {command}, direction: {direction}, stepSize: {step_size}, stepUnit: {step_unit}, acce: {acce_value}, dece: {dece_value}")
    ...
    try:
        ...
        if command in ["move", "step"]:
            ...
            # Safety check current position against limits if in ultra-safe mode
            with position_lock:
                next_position = current_position + final_step
                if ULTRASAFE_MODE and (next_position < -30 or next_position > 30):
                    logger.warning(f"Safety limit would be exceeded: {next_position} mm, limiting step")
                    # Adjust step to stay within limits
                    if next_position < -30:
                        final_step = -30 - current_position
                    elif next_position > 30:
                        final_step = 30 - current_position
    ...
    try:
        ...
        for i in range(100):  # Limit to 100 iterations
            ...
            try:
                ...
                if action == "step":
                    # Make a random step within safe limits
                    direction = random.choice([1, -1])
                    step_size = random.uniform(0.1, 2.0)  # Smaller steps for safety
                    
                    # Check if step would exceed limits
                    with position_lock:
                        next_position = current_position + (step_size * direction)
                        if next_position < -25 or next_position > 25:  # Keep extra margin
                    ...
                    logger.info(f"Demo: Step {step_size:.2f} mm {'right' if direction == 1 else 'left'}")
                    ...
                    # Ensure we don't scan outside limits
                    with position_lock:
                        if (direction == 1 and current_position > 20) or (direction == -1 and current_position < -20):
                            direction = -direction  # Reverse if near limits
    ...
    while not shutdown_requested:
        try:
            ...
            try:
                ...
                delay_factor = max(0.1, delay_factor * 0.95 + 0.05)  # Slowly increase performance
                
            except Exception as e:
                logger.error(f"Frame send error: {e}")
                delay_factor = min(1.0, delay_factor * 1.2)  # Back off slightly on error
                await asyncio.sleep(0.01)
        ...
        try:
            ...
            if websocket:
                try:
                    ...
                    logger.debug(f"Sent queued command: {command.get('type', 'unknown')} {command.get('command', '')}")
                except Exception as e:
    ...
    while not shutdown_requested:
        try:
            ...
            registration_message = {
                ...
                "status": "ready",
                "message": f"RPi {STATION_ID} combined connection initialized (Bulletproof v2.0)",
