    ...
    while not shutdown_requested:
        try:
            ...
            if elapsed < EPOS_UPDATE_INTERVAL:
                ...
                await asyncio.sleep(EPOS_UPDATE_INTERVAL - elapsed + MIN_SLEEP_DELAY)
...
async def rpi_client():
    ...
    logger.info(f"CPU usage optimization enabled with {MIN_SLEEP_DELAY*1000:.2f}ms minimum delays")
    ...
    while not shutdown_requested:
        try:
            ...
            logger.info(f"Retrying connection in {actual_delay:.1f} seconds (attempt {total_connection_failures})...")
            await asyncio.sleep(actual_delay)
            
            # If too many failures, restart hardware
            if total_connection_failures >= max_failures_before_reset:
                logger.warning(f"Too many connection failures ({total_connection_failures}), restarting hardware...")
