#!/usr/bin/env python3
"""
Backward-Compatible Ultra-Responsive RPi Client for Xeryon Demo Station
- Optimized for older websockets library versions
- Zero delay between button press and actuator movement
- Guaranteed display of movement in real-time
- Designed for maximum reliability with any websockets version
"""
...
async def process_command(data):
    ...
    try:
        ...
        if command in ["move", "step"]:
            ...
            # DIRECT EXECUTION: Try to use sendNoWaitCommand if available, otherwise fall back
            try:
                ...
                if hasattr(axis, 'sendNoWaitCommand'):
                    ...
                    # Fall back to direct command without waiting
...
async def send_position_updates(websocket):
    ...
    while not shutdown_requested:
        try:
            ...
            # Send with no timeout - older websockets library doesn't support wait_for
            try:
    ...
    while not shutdown_requested:
        try:
            ...
            try:
                ...
                # Simple send without timeout - compatible with older websockets
...
async def rpi_client():
    ...
    logger.info(f"Starting Backward-Compatible Ultra-Responsive RPi Client for {STATION_ID}")
    ...
    while not shutdown_requested:
        try:
            ...
            # IMPORTANT: Use the old-style connection without extra_headers for backward compatibility
            async with websockets.connect(
                ...
                registration_message = {
                    ...
                    "status": "ready",
                    "message": f"RPi {STATION_ID} connected (Backward-Compatible Ultra-Responsive Client)",
                ...
                try:
                    while not shutdown_requested:
                        # Get message with a manual timeout implementation to be backward compatible
                        try:
                            # Set up a wait for the next message
