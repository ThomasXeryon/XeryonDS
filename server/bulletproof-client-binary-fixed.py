...
- Binary WebSocket frame transmission for camera frames
"""
    ...
    try:
        ...
        if command == "step":
            ...
            with position_lock:
                ...
                try:
                    # Replace getPos() with getPos for compatibility
                    current_pos = axis.getPos()
                except Exception as e:
                    # Try alternative method
                    try:
                        response_text = axis.sendCommand("RPOS?")
                        if isinstance(response_text, str) and response_text.strip():
                            current_pos = float(response_text.strip())
                    except Exception as e2:
                        logger.warning(f"Error getting position from both methods: {e}, {e2}")
            ...
            try:
                ...
                with position_lock:
                    # Try to get position, use alternative if first method fails
                    try:
                        ...
                        try:
                            response_text = axis.sendCommand("RPOS?")
                            if isinstance(response_text, str) and response_text.strip():
                                current_position = float(response_text.strip())
                        except Exception as e2:
                            logger.warning(f"Error updating position: {e}, {e2}")
            ...
            try:
                ...
                for step in demo_positions:
                    ...
                    while time.time() - start_time < 10:  # 10 second timeout
                        ...
                        try:
                            # Check if we're at position - use alternative methods if needed
                            try:
                                current_pos = axis.getPos()
                            except Exception as e:
                                try:
                                    ...
                                    logger.warning(f"Error getting position: {e}, {e2}")
                                    current_pos = current_position
...
async def send_position_updates(websocket):
    ...
    while not shutdown_requested:
        try:
            ...
            if RUNNING_ON_RPI and axis:
                try:
                    # Try to use getPos first
                    try:
                        position_value = axis.getPos()
                    except Exception as e:
                        # Fall back to sendCommand
                        try:
                            response_text = axis.sendCommand("RPOS?")
                            if isinstance(response_text, str) and response_text.strip():
                                position_value = float(response_text.strip())
                        except Exception as e2:
                            logger.error(f"Error getting position from both methods: {e}, {e2}")
