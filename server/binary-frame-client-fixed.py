...
try:
    ...
    import cv2
    import serial
    # Import the Xeryon library - IMPORTANT: Update this path to match your setup
    sys.path.append('/home/pi/Desktop/RemoteDemoStation/BasicServer/Python')
    # Check for different Xeryon library versions and import accordingly
    try:
        from Xeryon import Xeryon, Stage, Units
        XERYON_HAS_STAGES = True
    except (ImportError, AttributeError):
        try:
            from Xeryon import Xeryon, XLSAxis, Units
            XERYON_HAS_STAGES = False
        except ImportError:
            print("ERROR: Could not import Xeryon library. Please check the path.")
            sys.exit(1)
    
    RUNNING_ON_RPI = True
except ImportError:
    RUNNING_ON_RPI = False
    XERYON_HAS_STAGES = False  # Default for simulation mode
...
# Protocol configuration for binary frames - FIXED: Use Unsigned integers for timestamps
    ...
    try:
        ...
        for attempt in range(max_tries):
            try:
                ...
                # Handle different Xeryon library versions
                if XERYON_HAS_STAGES:
                    # For API with 'stages' attribute
                    axis = controller.stages[0]
                else:
                    # For API with direct XLSAxis instantiation
                    axis = XLSAxis(controller)
    ...
    while not shutdown_requested:
        try:
            ...
            if not RUNNING_ON_RPI:
                ...
                timestamp = int(current_time * 1000) % 0xFFFFFFFF  # Ensure timestamp fits in 32-bit unsigned int
                header = struct.pack(FRAME_HEADER_FORMAT,
                                    STATION_ID.encode()[:4].ljust(4),
                                    frame_count % 0xFFFFFFFF,  # Ensure frame number fits in 32-bit unsigned int
                                    timestamp)
            ...
            try:
                ...
                # Create binary frame header - FIXED: Ensure values fit in unsigned 32-bit integers
                timestamp = int(time.time() * 1000) % 0xFFFFFFFF  # Milliseconds since epoch, capped to 32-bit
                frame_num = frame_count % 0xFFFFFFFF  # Ensure frame number wraps properly
    ...
    while not shutdown_requested:
        try:
            ...
            if current_time - last_ping_time >= CONNECTION_HEARTBEAT_INTERVAL:
                ...
                try:
                    logger.debug(f"> TEXT '{json.dumps(ping_message)}' [{len(json.dumps(ping_message))} bytes]")
